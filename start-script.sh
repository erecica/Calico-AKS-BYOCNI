...
echo "Resourcegroup created. Moving to the next step" && \
sleep 5 && \ 
...
echo "AKS without network-plugin created. This might take 6-9 minutes" && \
sleep 5 && \
...
sleep 30 && \
kubectl get svc -n yaobank-customer yaobank-customer --output jsonpath='{.status.loadBalancer.ingress[0].ip}'